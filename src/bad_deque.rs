...
impl<T> List<T> {
    ...
    pub fn peek_front(&self) -> Option<Ref<T>> {
        self.head.as_ref().map(|wrapped_node| {
            Ref::map(wrapped_node.borrow(), |node| &node.elem)
        })
    }

    pub fn peek_front_mut(&mut self) -> Option<RefMut<T>> {
        self.head.as_mut().map(|wrapped_node| {
            RefMut::map(wrapped_node.borrow_mut(), |node| &mut node.elem)
        })
    }

    pub fn peek_back(&self) -> Option<Ref<T>> {
        self.tail.as_ref().map(|wrapped_node| {
            Ref::map(wrapped_node.borrow(), |node| &node.elem) 
        })
    }

    pub fn peek_back_mut(&mut self) -> Option<RefMut<T>> {
        self.tail.as_mut().map(|wrapped_node| {
            RefMut::map(wrapped_node.borrow_mut(), |node| &mut node.elem) 
        })
    }
